module MbiconjGrad
	real(8), allocatable, dimension(:), private :: y, p, r, z !variables auxiliares de biconjGrad
contains 
subroutine biconjGrad(spMtx, spIdx, spRowptr, diagMtx, x, b, x_fix, x_fixIdx, npoin, nfix)
!--------------------------------------------------------------------------------------------------
...
!--------------------------------------------------------------------------------------------------
	...
	call vcopy2(npoin, 1.d30, p, nfix, x_fixIdx, y)
	py = vecdot(npoin, p, y)
	alfa = err_new/py
	call vecsum(npoin, alfa, p, x, x)
	!call daxpy(npoin, alfa, p, 1, x, 1)
	err_old = err_new

	do while(dabs(err_old) > tol .and. k < 1000)
		k = k + 1
		call vecsum(npoin, -alfa, y, r, r)
		!call daxpy(npoin, -alfa, r, 1, y, 1)
...
subroutine vassign2(n, y, m, idx, scal)
	implicit none
	integer n, m, i
	real(8) y(n), scal
	integer idx(m)
...
subroutine vcopy1(m, alfa, x, idx, n, y)
!-----------------------------------------
!			y(idx(:)) = alfa*x
!-----------------------------------------
	implicit none
...
subroutine vcopy2(n, alfa, x, m, idx, y)
!-----------------------------------------
!			y(idx(:)) = alfa*x(idx(:))
!-----------------------------------------
	implicit none
